...
public class GestionComisiones {
	...
	public static void main(String[] args) {
		...
		// crea la tienda
		Tienda tienda = new Tienda("C:\\Users\\samue\\Documents\\SOFTWARE 2\\Material Practica 5\\datosTienda.txt");
		...
		while (true) {
			...
			case NUEVA_VENTA:
				...
				try {
					if (!tienda.anhadeVenta(dni, importe)) {
						mensaje(MSJ_ERROR, "El vendedor no existe");
					}
				} catch (DataAccessException e) {
					mensaje(MSJ_ERROR, "No se pudo guardar el cambio");
				}
				break;
			...
			case VENDEDOR_DEL_MES:
				try {
					...
					resultado = calculaMejorEmpleado(tienda);
					...
					mensaje(MSJ_ERROR, "No se pudo acceder a los datos");
				}
				break;
			...
			default:
				mensaje(MSJ_ERROR, "No es una opcion válida");
				break;
			}
		}
	}

	private static List<Vendedor> calculaMejorEmpleado(Tienda tienda) throws DataAccessException {
		List<Vendedor> vendedores;
		List<Vendedor> resultado;
		vendedores = tienda.vendedores();
